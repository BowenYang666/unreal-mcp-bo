...
`INiagaraParticleCallbackHandler` (a UE Interface defined via `UINTERFACE`) on your UObject to
receive per-particle data. Since `ReceiveParticleData` is a `BlueprintNativeEvent`, override
the `_Implementation` variant:

```cpp
// Your class must implement INiagaraParticleCallbackHandler.
#include "NiagaraDataInterfaceExport.h"

// Note: INiagaraParticleCallbackHandler is a UE Interface (UINTERFACE/BlueprintNativeEvent).
// Override ReceiveParticleData_Implementation, not ReceiveParticleData directly.
UCLASS()
