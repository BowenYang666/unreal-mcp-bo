...
- The `Niagara` plugin is enabled (it is a built-in engine plugin, enabled by default).
- The target module's `Build.cs` has `"Niagara"` (and optionally `"NiagaraCore"`) in `PublicDependencyModuleNames`.
  - **This project**: `ThirdPersonTest1.Build.cs` already includes `"Niagara"` in `PublicDependencyModuleNames`.
